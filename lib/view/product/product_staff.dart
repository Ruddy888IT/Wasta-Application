...
class product_staff extends StatefulWidget {
   final AppBarName appbarname;
  product_staff({super.key, required this.works, required this.appbarname});
  String works;

  
  @override
  _product_staffState createState() => _product_staffState();
}

class _product_staffState extends State<product_staff> {
  ...
  Future<void> fetchItems() async {
    ...
    final List<Item> itemList = (await itemsRef.where('Service', isEqualTo: 'ستافەکان').get()).docs
        .map((doc) => Item(name: doc['Name'], imageUrl: doc['Images'], city: doc['City'],work: doc['Work']))
    
        .toList();
    // final List<String> urlList = await Future.wait(itemList.map((item) =>
